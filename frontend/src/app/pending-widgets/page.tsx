...
import { fetchPendingWidgets, approveWidget, declineWidget, Request } from '../../api/widget';
...
export default function PendingWidgetsPage() {
  ...
  const handleApproveWidget = async (requestId: string) => {
    try {
      ...
      setRequests((prevWidgets) => prevWidgets.filter(request => request.request_id !== requestId));
    } catch (error: any) {
  ...
  return (
    <div className="container mx-auto p-8">
        ...
        {status && (
            <p className="text-center text-green-600 mb-4 font-semibold">{status}</p>
        )}
