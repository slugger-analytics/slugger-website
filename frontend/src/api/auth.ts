export async function signUpUser(data: { email: string, password: string, firstName: string, lastName: string, role:string }) {
    try {
        console.log(data)
        const response = await fetch('http://localhost:3001/api/register-user', {  // Update to your backend URL
            method: 'POST',
            headers: {
