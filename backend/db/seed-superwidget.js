/**
 * Inserts the SuperWidget Analyzer as an approved public widget
 * with redirect_link pointing to the /super-widget page.
 *
...
async function seedSuperWidget() {
  ...
  try {
    ...
    // Check if SuperWidget already exists to avoid duplicates
    ...
    if (existing.rowCount > 0) {
      console.log('SuperWidget already exists (widget_id:', existing.rows[0].widget_id, '). Skipping.');
    ...
    console.log('Done. SuperWidget is now visible in the Widgets section.');
  } catch (error) {
    await client.query('ROLLBACK');
